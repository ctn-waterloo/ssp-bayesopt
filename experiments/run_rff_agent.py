"""Run Bayesian optimization trials using the RFF agent with optional nengo backend.

Usage:
    python run_rff_agent.py --func himmelblau --agent rff --num-samples 100
"""
import numpy as np
import pytry
import time
...
try:
    import nengo_loihi
    _loihi_lif = nengo_loihi.LoihiLIF()
    _loihi_sim = nengo_loihi.Simulator
    _loihi_allocator = nengo_loihi.hardware.allocators.Greedy()
except ImportError:
    print('Failed to import nengo_loihi')
    _loihi_lif = None
    _loihi_sim = None
    _loihi_allocator = None
...
sim_types = {
    'cpu': (nengo.Simulator, {'progress_bar': False}),
    'loihi-sim': (_loihi_sim, {'progress_bar': False, 'target': 'sim'}),
    'loihi': (_loihi_sim,
              {'progress_bar': False,
               'target': 'loihi', 'precompute': True,
               'hardware_options': {
                   "snip_max_spikes_per_step": 300,
                   "allocator": _loihi_allocator,
                   "n_chips": 15
               }})
}
...
if __name__ == '__main__':
    ...
    if args.nengo:
        data_path = os.path.join(os.getcwd(),
                                 args.data_dir, args.function_name,
                                 args.agent_type + '_nengo-' + args.backend)
    else:
        data_path = os.path.join(os.getcwd(),
