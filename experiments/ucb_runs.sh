...
if [ "$run_index" -eq 0 ]; then
    ...
    python run_agent.py --agent "ssp-rand" --num-trials 30 --func $func --data-dir $dest_dir --len-scale -1 --ssp-dim $sspdim --gamma 0.0 --beta-ucb 0.1
elif [ "$run_index" -eq 1 ]; then
    echo "Running ssp-hex"
    python run_agent.py --agent "ssp-hex" --num-trials 30 --func $func --data-dir $dest_dir --len-scale -1 --ssp-dim $sspdim --gamma 0.0 --beta-ucb 0.1
elif [ "$run_index" -eq 2 ]; then
    echo "Running gp-sinc"
    python run_agent.py --agent "gp-sinc" --num-trials 30 --func $func --data-dir $dest_dir --gamma 0.0 --beta-ucb 0.1
elif [ "$run_index" -eq 3 ]; then
    echo "Running gp-matern"
    python run_agent.py --agent "gp-matern" --num-trials 30 --func $func --data-dir $dest_dir --gamma 0.0 --beta-ucb 0.1
