...
folder = os.path.abspath(os.path.join(os.getcwd(), os.pardir)) + '/data/dim-sensitivity/'
funcs = ["branin-hoo"]
func=funcs[0]
agts=[ "ssp-hex" , "ssp-rand" ]
labels=['SSP-BO-Hex', 'SSP-BO-Rand','GP-MI-sinc', 'GP-MI-Matern']
dims =  [7, 25,55,97,151,217,295]
...
other_plot_data=[]
for j, agt in enumerate(["gp-sinc", "gp-matern"]):
    data = pd.DataFrame(read(os.path.abspath(os.path.join(os.getcwd(), os.pardir)) + '/data/test-funcs/branin-hoo/' + agt))
    ...
    plt_data = get_mean_and_ci( np.array(regrets).reshape(-1,1))
    ax.fill_between(dims, plt_data["upper_bound"]*np.ones(len(dims)), plt_data["lower_bound"]*np.ones(len(dims)),
                    alpha=.2, color=cols[j+2])
    ax.plot(dims, plt_data["mean"]*np.ones(len(dims)), color=cols[j+2], label=labels[j+2], linestyle=linestys[j+2])
ax.set_title('Terminal Average Regret vs SSP Dimension\n (Branin-Hoo, ' + str(num_trials) +' trials)')

means = []
for j,agt in enumerate(agts):
    regret_vs_dim = []
    for i,dim in enumerate(dims):
        data = pd.DataFrame(read(folder + str(dim) + '/' + func + '/' + agt))
        regrets= np.array([data['regret'][i] for i in range(len(data['regret']))])
        budget=regrets.shape[1]
        num_trials=regrets.shape[0]
        regrets = np.divide(np.cumsum(regrets, axis=1), matlib.repmat(range(1,budget+1), num_trials,1))[:,-1]
...
ax.set_xlabel("SSP Dimension ($d$)")
ax.set_ylabel("Terminal Regret (a.u.)")
ax.legend()


utils.save(fig, 'test-func_dims.pdf')
