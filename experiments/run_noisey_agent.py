...
def get_args():
    ...
    parser.add_argument('--data-dir', dest='data_dir', type=str, default='data/noise-exp')
...
class SamplingTrial(pytry.Trial):
    ...
    def evaluate(self, p):        
        _target, pbounds, _ = functions.factory(p.function_name)
        rng = np.random.RandomState(p.seed)

        def target(x,info=None):
            y = _target(x)
            return y + rng.normal(0, p.sigma, size=y.shape)
        ...
        for i, res in enumerate(optimizer.res):
            ...
            vals[i] = target(np.atleast_2d(sample_locs[-1]))
