import matplotlib.pyplot as plt
import seaborn as sns
import time
import numpy as np

import ssp_bayes_opt

import importlib
importlib.reload(ssp_bayes_opt)

# Apply the default theme
sns.set_theme()



class ContexualFunction:

    def __init__(self):
        self.data_dim = 1
        self.context_size = 2
        self.dim = self.data_dim + 1
        self.bounds = np.array([[0, 1.2], [0,self.context_size]])
        self.budget = 500

    def reset(self):
        return np.array([[0]])
        
    def __call__(self, xc, info):
        x = xc[:,:self.data_dim]
        context = xc[:,-1]

        if context == 0:
            return (1.4 - 3.0 * x) * np.sin(18.0 * x), np.array([[1]])
        elif context == 1:
            return x*np.sin(x*3+2) + 0.5, np.array([[0]])
        else:
            raise ValueError('Invalid context. context must be an integer from 0 to ' +
                             str(self.context_size) + ' and instead it was ' + str(context))

    def call_vectorized(self, xc, info):
        x = xc[:,:self.data_dim].reshape(-1)
        context = xc[:,-1].reshape(-1)
        
        # return - ((x[:,0]**2 + x[:,1] - 11)**2 + (x[:,0] + x[:,1]**2 - 7)**2 + x[:,0] + x[:,1]) / 100, np.array([[-1]])
        res = np.zeros((x.shape[0]))
        res_c = np.zeros((x.shape[0]))
        res[context==0] = (1.4 - 3.0 * x[context==0]) * np.sin(18.0 * x[context==0])
        res_c[context==0] = 1
        
        res[context==1] = x[context==1]*np.sin(x[context==1]*3+2) + 0.5
        res_c[context==1] = 0
        return res, res_c

       


target = ContexualFunction()
pbounds = target.bounds
budget = 400

optimizer = ssp_bayes_opt.BayesianOptimization(f=target,
                                               bounds=pbounds, 
                                               verbose=True,
                                               sampling_seed=0)
num_init_samples=40
start = time.thread_time_ns()
optimizer.maximize(init_points=num_init_samples, 
                   n_iter=budget,
                   num_restarts=1,
                   agent_type='ssp-discrete-context',
                   ssp_dim=200,
                   decoder_method='from-set')

elapsed_time = time.thread_time_ns() - start

vals = np.zeros((num_init_samples + budget,))
sample_locs = []

for i, res in enumerate(optimizer.res):
    vals[i] = res['target']
    sample_locs.append(res['params'])
    

con1 = [l[-1] == 0 for l in sample_locs]
con2 = [l[-1] == 1 for l in sample_locs]
masks = [con1, con2]

_xsamples =np.array(sample_locs)
_ysamples = np.array(vals)

samples_per_dim = 100
_xs = np.linspace(pbounds[0,0],pbounds[0,1], samples_per_dim).reshape(-1,1)
ctxts = [0,1]
fig = plt.figure(figsize=plt.figaspect(0.5))
for i, context in enumerate(ctxts):
    xs = np.hstack([_xs,context*np.ones((_xs.shape[0],1))])
    phis = optimizer.agt.encode(xs)
    mu, var = optimizer.agt.blr.predict(phis)
    
    xsamples =_xsamples[masks[i],:-1]
    ysamples = _ysamples[masks[i]]
    res, _ = target.call_vectorized(xs, None)
    
    ax = fig.add_subplot(1, 2, i+1)
    plt.plot(_xs,res, "-", color="grey")
    plt.plot(_xs.reshape(-1), mu.reshape(-1), color=sns.color_palette()[0])
    plt.fill_between(_xs.reshape(-1),  mu.reshape(-1)-np.sqrt(var), 
                     mu.reshape(-1)+np.sqrt(var), alpha=0.5, edgecolor=sns.color_palette()[0],
                     facecolor=sns.color_palette()[0])
   
    plt.plot(np.array(sample_locs)[masks[i],0].reshape(-1), np.array(vals)[masks[i]], "ro")
        
