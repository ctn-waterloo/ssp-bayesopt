...
class SSPMultiAgent(SSPAgent):
    ...
    def _set_ssp_space(self, n_agents,
                       ...
                       seed=0,
                       init_pos=None,
                       same_agt_x_space=False,
                       same_agt_t_space=False,
                       **kwargs):
        assert n_agents > 0
        ...
        if ssp_t_space is None:
            if same_agt_t_space:
                ...
                ssp_t_spaces = [ssp_t_space] * n_agents

            else:
                ssp_t_spaces=[]
                for i in range(n_agents):
                    ssp_t_spaces.append( sspspace.RandomSSPSpace(1, ssp_dim=ssp_x_spaces[0].ssp_dim,
                                                  domain_bounds=np.array([[0, self.traj_len+1]]),
                                                  length_scale=1, rng=n_agents+seed+i))


        self.ssp_dim = ssp_x_spaces[0].ssp_dim
        self.ssp_x_spaces = ssp_x_spaces
        self.ssp_t_spaces = ssp_t_spaces
        ...
        if not 'length_scale' in kwargs or np.any(np.array(kwargs.get('length_scale')) < 0):
            ...
            for i in range(n_agents):
                ...
                self.ssp_t_spaces[i].update_lengthscale(optres[-1])
        else:
            for i in range(n_agents):
                self.ssp_x_spaces[i].update_lengthscale(kwargs.get('length_scale', 4))
                self.ssp_t_spaces[i].update_lengthscale(kwargs.get('time_length_scale', 1))
        ...
        if same_agt_t_space:
            timestep_ssps = self.ssp_t_spaces[0].encode(timesteps)
            timestep_inv_ssps = self.ssp_t_spaces[0].encode(-timesteps)
            self.timestep_ssps = [timestep_ssps] * n_agents
            self.timestep_inv_ssps =[timestep_inv_ssps] * n_agents
        else:
            self.timestep_ssps = []
            self.timestep_inv_ssps = []
            for i in range(n_agents):
                self.timestep_ssps.append(self.ssp_t_spaces[i].encode(timesteps))
                self.timestep_inv_ssps.append(self.ssp_t_spaces[i].encode(-timesteps))
    ...
    def encode(self,x,timestep_ssps=None):
        ...
        for i in range(self.n_agents):
            ...
            for j in range(self.traj_len):
                Si = Si + self.ssp_x_spaces[i].bind(timestep_ssps[i][j,:],
    ...
    def decode(self,ssp,timestep_ssps=None):
        ...
        for i in range(self.n_agents):
            ...
            queries = self.ssp_x_spaces[i].bind(timestep_inv_ssps[i], sspi)
            decoded_traj[i,:,:] = self.ssp_x_spaces[i].decode(queries,
                                                              method=self.decoder_method,
                                                              samples=self.init_samples[i])
            # Si = self.ssp_x_spaces[i].bind(timestep_ssps,
            #                                     self.ssp_x_spaces[i].encode(decoded_traj[i,:,:]))
            # ssp = ssp - self.ssp_x_spaces[i].bind(self.agent_sps[i,:], Si)
            # for j in range(self.traj_len):
