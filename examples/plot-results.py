...
if __name__ == '__main__':
    ...
    func_folders = os.listdir(args.path)
...
#     exit()
    ...
    for func_name in func_folders:
#         func_name='himmelblau'
        gp_data = pd.DataFrame(pytry.read(args.path + func_name + '/gp-mi'))
        gp_regret, gp_time = get_data(gp_data)

        ssp_data = pd.DataFrame(pytry.read(args.path + func_name + '/ssp-mi'))
        ssp_regret, ssp_time = get_data(ssp_data)

        hex_data = pd.DataFrame(pytry.read(args.path + func_name + '/hex-mi'))
        hex_regret, hex_time = get_data(hex_data)

        (num_trials, num_obs) = gp_regret.shape
       


        (gp_num_trials, budget1) = gp_regret.shape
        (ssp_num_trials, budget2) = ssp_regret.shape
        (hex_num_trials, budget3) = hex_regret.shape
        budget = np.min([budget1,budget2,budget3])
        gp_regret = gp_regret[:,:budget]
        ssp_regret = ssp_regret[:,:budget]
        hex_regret = hex_regret[:,:budget]
...
#         plt.subplot(2, 1, 1)
        ...
        plt.title(f'Average Regret for {func_name.title()}')
...
#         plt.subplot(2, 1, 2)
        ...
        plt.xlabel('Sample Number')#, fontsize=24)
        plt.title(f'Query Time vs Sample Number, {func_name.title()}')#, fontsize=18)
        plt.tight_layout()
